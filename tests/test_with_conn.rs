    ...
    // Transaction with write should fail
    {
        ...
        match tx {
            Ok(tx) => {
                ...
                assert!(
                    result.is_err(),
                    "insert in transaction should fail when fenced"
                );
            }
            Err(_) => {
...
async fn fence_blocks_pragma_writes() {
    ...
    let guard = conn.lock();
    assert!(
        guard.execute_batch("PRAGMA user_version = 42").is_err(),
        "PRAGMA write should be blocked"
    );
}
...
async fn fence_blocks_replace() {
    ...
    assert!(
        guard
            .execute("REPLACE INTO users (id, name) VALUES (1, 'Bob')", [])
            .is_err(),
