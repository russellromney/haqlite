//! HTTP `StorageBackend` adapter for turbolite.
//!
//! This adapter lives in haqlite (not turbolite) because it knows about HA
//! concerns that turbolite shouldn't: fence tokens, Bearer auth, and the
//! specific `/v1/sync/pages/` API contract. turbolite exposes
//! `hadb_storage::StorageBackend`; we implement it here and plug it in via
//! `TurboliteVfs::new_with_storage`.
//!
//! # Fence-token contract
//!
//! Every write carries a `Fence-Token` header matching the writer's current
//! lease revision. If the shared fence token is 0 (lease not yet acquired,
//! or lost) we refuse to write rather than silently issuing fence-less PUTs
//! that a server might accept and then reject for a former-leader.
//!
//! # Server contract
//!
//! Speaks turbolite-style relative keys (`manifest.msgpack`, `p/d/0_v1`,
//! `p/it/3_v5`, ...) against a server that scopes keys by Bearer token.
//!
//! # Status
//!
//! This adapter is superseded by `hadb_storage_cinch::CinchHttpStorage`
//! (phase Anvil h). Kept for one step while haqlite finishes migrating
//! its builder to take `Arc<dyn StorageBackend>` directly.

use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::Arc;

use anyhow::{anyhow, Result};
use async_trait::async_trait;
use hadb_storage::{CasResult, StorageBackend};

/// HTTP-backed `StorageBackend` with fence-token fencing on writes.
pub struct HttpPageStorage {
    client: reqwest::Client,
    endpoint: String,
    token: String,
    /// Shared fence token: updated by the lease renewal loop. Writes use this
    /// value as the `Fence-Token` header; zero means "no active lease" and
    /// blocks writes.
    fence_token: Arc<AtomicU64>,
    fetch_count: AtomicU64,
    fetch_bytes: AtomicU64,
}

impl HttpPageStorage {
    pub fn new(endpoint: &str, token: &str, fence_token: Arc<AtomicU64>) -> Self {
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(30))
            .build()
            .expect("failed to build HTTP client");
        Self {
            client,
            endpoint: endpoint.trim_end_matches('/').to_string(),
            token: token.to_string(),
            fence_token,
            fetch_count: AtomicU64::new(0),
            fetch_bytes: AtomicU64::new(0),
        }
    }

    fn url(&self, key: &str) -> String {
        format!("{}/v1/sync/pages/{}", self.endpoint, key)
    }

    fn require_fence_header(&self) -> Result<String> {
        let fence = self.fence_token.load(Ordering::SeqCst);
        if fence == 0 {
            return Err(anyhow!(
                "Fence-Token is 0 (no active lease); refusing to write"
            ));
        }
        Ok(fence.to_string())
    }

    pub fn fetch_count(&self) -> u64 {
        self.fetch_count.load(Ordering::Relaxed)
    }

    pub fn fetch_bytes(&self) -> u64 {
        self.fetch_bytes.load(Ordering::Relaxed)
    }
}

#[async_trait]
impl StorageBackend for HttpPageStorage {
    async fn get(&self, key: &str) -> Result<Option<Vec<u8>>> {
        let mut retries = 0u32;
        loop {
            let resp = self
                .client
                .get(&self.url(key))
                .bearer_auth(&self.token)
                .send()
                .await
                .map_err(|e| anyhow!("HTTP GET {key}: {e}"))?;

            match resp.status().as_u16() {
                200 => {
                    let bytes = resp.bytes().await.map_err(|e| anyhow!(e))?;
                    self.fetch_count.fetch_add(1, Ordering::Relaxed);
                    self.fetch_bytes
                        .fetch_add(bytes.len() as u64, Ordering::Relaxed);
                    return Ok(Some(bytes.to_vec()));
                }
                404 => return Ok(None),
                status => {
                    retries += 1;
                    if retries >= 3 {
                        return Err(anyhow!(
                            "HTTP GET {key} returned {status} after 3 retries"
                        ));
                    }
                    tokio::time::sleep(std::time::Duration::from_millis(100 * (1 << retries)))
                        .await;
                }
            }
        }
    }

    async fn put(&self, key: &str, data: &[u8]) -> Result<()> {
        let fence = self.require_fence_header()?;
        let mut retries = 0u32;
        loop {
            let resp = self
                .client
                .put(&self.url(key))
                .bearer_auth(&self.token)
                .header("Fence-Token", fence.as_str())
                .body(data.to_vec())
                .send()
                .await
                .map_err(|e| anyhow!("HTTP PUT {key}: {e}"))?;
            if resp.status().is_success() {
                return Ok(());
            }
            retries += 1;
            if retries >= 3 {
                return Err(anyhow!(
                    "HTTP PUT {key} failed after 3 retries: {}",
                    resp.status()
                ));
            }
            tokio::time::sleep(std::time::Duration::from_millis(100 * (1 << retries))).await;
        }
    }

    async fn delete(&self, key: &str) -> Result<()> {
        let fence = self.require_fence_header()?;
        // Single-key delete: the server's batch delete endpoint accepts a keys array.
        let url = format!("{}/v1/sync/pages/_delete", self.endpoint);
        let resp = self
            .client
            .post(&url)
            .bearer_auth(&self.token)
            .header("Fence-Token", &fence)
            .json(&serde_json::json!({ "keys": [key] }))
            .send()
            .await
            .map_err(|e| anyhow!("HTTP DELETE {key}: {e}"))?;
        if !resp.status().is_success() {
            return Err(anyhow!(
                "HTTP DELETE {key} returned {}",
                resp.status()
            ));
        }
        Ok(())
    }

    async fn delete_many(&self, keys: &[String]) -> Result<usize> {
        if keys.is_empty() {
            return Ok(0);
        }
        let fence = self.require_fence_header()?;
        let url = format!("{}/v1/sync/pages/_delete", self.endpoint);
        let resp = self
            .client
            .post(&url)
            .bearer_auth(&self.token)
            .header("Fence-Token", &fence)
            .json(&serde_json::json!({ "keys": keys.to_vec() }))
            .send()
            .await
            .map_err(|e| anyhow!("HTTP batch delete: {e}"))?;
        if !resp.status().is_success() {
            return Err(anyhow!(
                "HTTP batch delete returned {}",
                resp.status()
            ));
        }
        Ok(keys.len())
    }

    async fn list(&self, _prefix: &str, _after: Option<&str>) -> Result<Vec<String>> {
        // The Cinch /v1/sync/pages/ endpoint is a key-value surface; there is
        // no native list. `hadb_storage_cinch::CinchHttpStorage` (step h) is
        // the adapter for callers that need listing. haqlite paths using this
        // shim never list.
        Err(anyhow!("list not supported on legacy HttpPageStorage shim"))
    }

    async fn put_if_absent(&self, _key: &str, _data: &[u8]) -> Result<CasResult> {
        Err(anyhow!(
            "put_if_absent not supported on legacy HttpPageStorage shim"
        ))
    }

    async fn put_if_match(&self, _key: &str, _data: &[u8], _etag: &str) -> Result<CasResult> {
        Err(anyhow!(
            "put_if_match not supported on legacy HttpPageStorage shim"
        ))
    }

    async fn range_get(
        &self,
        key: &str,
        start: u64,
        len: u32,
    ) -> Result<Option<Vec<u8>>> {
        let range = format!("bytes={}-{}", start, start + len as u64 - 1);
        let mut retries = 0u32;
        loop {
            let resp = self
                .client
                .get(&self.url(key))
                .bearer_auth(&self.token)
                .header("Range", &range)
                .send()
                .await
                .map_err(|e| anyhow!("HTTP range GET {key}: {e}"))?;

            match resp.status().as_u16() {
                200 | 206 => {
                    let bytes = resp.bytes().await.map_err(|e| anyhow!(e))?;
                    self.fetch_count.fetch_add(1, Ordering::Relaxed);
                    self.fetch_bytes
                        .fetch_add(bytes.len() as u64, Ordering::Relaxed);
                    return Ok(Some(bytes.to_vec()));
                }
                404 => return Ok(None),
                status => {
                    retries += 1;
                    if retries >= 3 {
                        return Err(anyhow!(
                            "HTTP range GET {key} ({range}) returned {status} after 3 retries"
                        ));
                    }
                    tokio::time::sleep(std::time::Duration::from_millis(100 * (1 << retries)))
                        .await;
                }
            }
        }
    }

    async fn put_many(&self, entries: &[(String, Vec<u8>)]) -> Result<()> {
        if entries.is_empty() {
            return Ok(());
        }
        let fence = self.require_fence_header()?;
        let mut handles = Vec::with_capacity(entries.len());
        for (key, data) in entries {
            let client = self.client.clone();
            let url = self.url(key);
            let token = self.token.clone();
            let data = data.clone();
            let fence = fence.clone();
            handles.push(tokio::spawn(async move {
                let mut retries = 0u32;
                loop {
                    let resp = client
                        .put(&url)
                        .bearer_auth(&token)
                        .header("Fence-Token", fence.as_str())
                        .body(data.clone())
                        .send()
                        .await
                        .map_err(|e| anyhow!(e))?;
                    if resp.status().is_success() {
                        return Ok::<_, anyhow::Error>(data.len() as u64);
                    }
                    retries += 1;
                    if retries >= 3 {
                        return Err(anyhow!(
                            "HTTP PUT failed after 3 retries: {}",
                            resp.status()
                        ));
                    }
                    tokio::time::sleep(std::time::Duration::from_millis(100 * (1 << retries)))
                        .await;
                }
            }));
        }
        for handle in handles {
            match handle.await {
                Ok(Ok(_)) => {}
                Ok(Err(e)) => return Err(e),
                Err(e) => return Err(anyhow!(e)),
            }
        }
        Ok(())
    }

    fn backend_name(&self) -> &str {
        "haqlite-http-shim"
    }
}
